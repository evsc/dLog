...
exports.initialize = function() {
		...
		db.Character.find( query , ['name','sentiment', 'dreams'], 
		  ...
		  if(docs) {
		  	docs.forEach( function (c) {
		  		// if(c.dreams.length > 5 && c.name != "DanielWilson") {
		  		if(c.dreams.length > 7) {
	...
	all = function(db,tag,character,sort,cflags,afterDate,beforeDate,response) {
		util.log("POST 'all'");
		util.log("cflags: "+cflags);
		util.log("afterDate: "+afterDate);
		util.log("beforeDate: "+beforeDate);

		var all = { allDreams : []};	
		var all_tags = [];
		var all_chars = [];

		// var afterDate = new Date(2013,1,1);
		// var beforeDate = new Date(2024,12,1);

		var query = {};
		if(tag=="false") tag = false;
		if(character=="false") character = false;
		if(afterDate=="false") afterDate = false 
			else afterDate = new Date(decodeURIComponent(afterDate));
		if(beforeDate=="false") beforeDate = false 
			else beforeDate = new Date(decodeURIComponent(beforeDate));
		if(tag) query['tags.name'] = tag;
		if(character) query['characters.name'] = character;
		if(afterDate || beforeDate) {
	        query['date'] = {};
	        if(afterDate) query['date']['$gt'] = new Date(afterDate);
  ...
  // 				{$sort:{size:1}}], 
	...
	sentimentAnalysis = function (odoc, callback) {
		...
		console.log("  comparative: "+result.comparative.toFixed(2));
		console.log("  magnitude: "+odoc.sentiment.magnitude.toFixed(2)+"\n");
