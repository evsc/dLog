...
exports.initialize = function() {
	...
	all = function(db,tag,character,response) {
		...
		db.Dream.find( query , ['title','date','start','characters','tags'], function (err, docs) {
		  if(err) util.log("error: "+err.message);
		  if(docs) {
		  	docs.forEach( function (d) {

		  		// clean up dream, so we don't need to transmit all tags and characters
		  		var stripDream = { _id: d._id, title: d.title, date: d.date, start: d.start, tag_cnt: d.tags.length, char_cnt: d.characters.length };
